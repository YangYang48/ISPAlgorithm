...
for i = 1: m
    for j = 1: n
        ...
        % refer to "Noise Reduction Techniques for Bayer-Matrix Images"
        DenoisedImg(i, j) = CWMF + (median_HV + median_diag - 2 * CWMF) / (h + k * (median_HV - median_diag));
        
        % refer to "MEDIAN-RATIONAL HYBRID FILTERS Laxhar"
        % DenoisedImg(i, j) = CWMF + (median_HV + median_diag - 2 * CWMF) / (h + k * (median_HV - median_diag)^2); 
        
        % Computational acceleration through L1 distance
        % DenoisedImg(i, j) = CWMF + (median_HV + median_diag - 2 * CWMF) / (h + k * abs(median_HV - median_diag));
    end
end
toc
figure();
